window.onload = function () {
  /*
      ...
      */
  const kadoTsuyo = [
    25, 10, 10, 10, 10, 10, 10, 25, 10, 5, 5, 5, 5, 5, 5, 10, 10, 5, 2, 2, 2, 2,
    5, 10, 10, 5, 2, 1, 1, 2, 5, 10, 10, 5, 2, 1, 1, 2, 5, 10, 10, 5, 2, 2, 2,
    2, 5, 10, 10, 5, 5, 5, 5, 5, 5, 10, 25, 10, 10, 10, 10, 10, 10, 25,
  ];
  ...
  const searchSpecificDirection = (x, y, direction, which) => {
    ...
    for (howManySearches = 1; judge == 1; howManySearches++) {
      ...
      if (subjectCell == null) {
        ...
        if (
          subjectCell.style.backgroundColor == colorArray[(which - 1) * -1 + 1]
        ) {
          //調べるマスの色が相手の色だったら、
          judge = 1;
          //その先のマスを調べる
  ...
  const searchSurroundingEight = (xx, yy, which) => {
    ...
    for (let b = -1; b <= 1; b++) {
      for (let a = -1; a <= 1; a++) {
        if (
          toCell(xx + a, yy + b).style.backgroundColor ==
          colorArray[(which - 1) * -1 + 1]
        ) {
          //調べるマスに相手の色の石が存在したら、
          ans.push(1);
          //配列ansに1を追加。
  ...
  const finishGame = () => {
    ...
    if (redPoints === greenPoints) {
      document.getElementById("finalResult").innerText = "引き分け！！";
    } else {
      document.getElementById(
        "finalResult"
      ).innerText = `赤・・・${redPoints} 点、緑・・・${greenPoints} 点で、
